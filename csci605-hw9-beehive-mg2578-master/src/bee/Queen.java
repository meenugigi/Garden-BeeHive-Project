...
public class Queen extends Bee {
    ...
    public void run() {
        // TODO YOUR CODE HERE
//      mate if resources present and beehive is active
        while(beeHive.isActive()){
            if(beeHive.getRemainingNectar() > 0 && beeHive.getRemainingPollen() > 0){
                if(beeHive.getQueensChamber().hasDrone()){
                    beeHive.getQueensChamber().isReady = true;
//                    removes drone from queue.
...
//                    generates the number of bees to be craeted.
                    int randomBeeCreate = RandomBee.nextInt(MIN_NEW_BEES,
                            MAX_NEW_BEES);
                    createNewBees(randomBeeCreate);
                    System.out.println("*Q* Queen birthed #" + randomBeeCreate +
                            " children");
...
//        individually dismisses each drone that is
//        still waiting in her chamber
        beeHive.getQueensChamber().dismissDrone();
    }


    /** The bees are created based on
     * another random dice roll - a nectar worker bee has a 20% chance
     * of being created, a pollen bee has a 20% change of being created,
     * and a drone has a 60% change of being created.
     *
     * @param randomBeeCreate   no of bees to be created.
     */
    private void createNewBees(int randomBeeCreate) {
        for(int i = 0; i < randomBeeCreate; i++){
            if(beeHive.getRemainingPollen() >= 1 && beeHive.getRemainingNectar() >= 1){
                int randomProbability = RandomBee.nextInt(1, 10);
//                if 20% probabibilty, create nectar
                if(randomProbability == 1 || randomProbability == 2){
                    beeHive.addBee(createBee(Role.WORKER,
                            Worker.Resource.NECTAR,
                            beeHive));
                }
//                if 20% probabibilty, create pollen
                else if(randomProbability == 3 || randomProbability == 4){
                    beeHive.addBee(createBee(Role.WORKER, Worker.Resource.POLLEN,
                            beeHive));
                }
//                if 60% probabibilty, create drone
                else{
                    beeHive.addBee(createBee(Role.DRONE, Worker.Resource.NONE,
                            beeHive));
                }
//                claim resources required for creating bees.
