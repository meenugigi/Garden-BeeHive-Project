...
public class BeeHive {
    ...
    public void begin() {
        ...
        //starting all the bees in the concurrentlinkedqueue
        for(Bee bee: bees){
            bee.start();
        }
    }
...
//        start the bees
    ...
    public synchronized boolean hasResources() {
        // TODO YOUR CODE HERE
        if(getPollenGathered() >= 1 && getNectarGathered() >= 1){
            return true;
        }
        return false;
    }
...
//        deposists resources collcted and increments the count respectively.
        switch (resource){
            case NECTAR -> {
                nectar ++;
                nectarGathered++;
